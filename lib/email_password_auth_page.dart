  ...
  Future<void> submit() async {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        if (!mounted) return;
        setState(() => notice = response.session != null ? 'Account created as @$username.' : 'Account created as @$username. Check your email to confirm your account.');
      } else {
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  ...
                  Text(forgotPassword ? 'Reset your password' : isSignUp ? 'Create your account' : 'Sign in with your email and password', textAlign: TextAlign.center),
                  ...
                  if (!forgotPassword && isSignUp) ...[
                    ...
                    TextField(controller: usernameController, enabled: !loading, autocorrect: false, textCapitalization: TextCapitalization.none, decoration: const InputDecoration(labelText: 'Username', hintText: 'yourname', prefixText: '@', prefixIcon: Icon(Icons.alternate_email_rounded))),
                  ],
...
class PasswordRecoveryPage extends StatefulWidget {
  ...
  @override State<PasswordRecoveryPage> createState() => _PasswordRecoveryPageState();
}
...
class _PasswordRecoveryPageState extends State<PasswordRecoveryPage> {
  ...
  Future<void> updatePassword() async {
    ...
    if (password.length < 8) { setState(() => error = 'Your new password must be at least 8 characters.'); return; }
    if (password != confirm) { setState(() => error = 'Passwords do not match.'); return; }
    setState(() { loading = true; error = null; notice = null; });
    try {
      await Supabase.instance.client.auth.updateUser(UserAttributes(password: password));
      if (mounted) setState(() => notice = 'Password changed successfully.');
    } on AuthException catch (e) {
      if (mounted) setState(() => error = e.message);
    } catch (_) {
        ...
        child: Center(
          child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: LinearGradient(colors: [scheme.primary, scheme.tertiary]), shape: BoxShape.circle), child: Icon(Icons.lock_reset_rounded, size: 54, color: scheme.onPrimary)),
                  const SizedBox(height: 18),
                  Text('Reset password', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w900)),
                  const SizedBox(height: 8),
                  const Text('Choose a new password for your GG Messenger account.', textAlign: TextAlign.center),
                  const SizedBox(height: 30),
                  TextField(controller: passwordController, enabled: !loading, obscureText: obscure, decoration: InputDecoration(labelText: 'New password', prefixIcon: const Icon(Icons.lock_rounded), suffixIcon: IconButton(onPressed: () => setState(() => obscure = !obscure), icon: Icon(obscure ? Icons.visibility : Icons.visibility_off)))),
                  const SizedBox(height: 12),
                  TextField(controller: confirmController, enabled: !loading, obscureText: obscure, decoration: const InputDecoration(labelText: 'Confirm new password', prefixIcon: Icon(Icons.lock_outline_rounded))),
                  const SizedBox(height: 18),
                  if (notice != null) Padding(padding: const EdgeInsets.only(bottom: 12), child: Text(notice!, textAlign: TextAlign.center, style: TextStyle(color: scheme.primary, fontWeight: FontWeight.w700))),
                  if (error != null) Padding(padding: const EdgeInsets.only(bottom: 12), child: Text(error!, textAlign: TextAlign.center, style: TextStyle(color: scheme.error))),
                  SizedBox(width: double.infinity, height: 54, child: FilledButton.icon(onPressed: loading ? null : updatePassword, icon: const Icon(Icons.check_circle_rounded), label: Text(loading ? 'Saving...' : 'Set new password'))),
                ],
              ),
            ),
          ),
