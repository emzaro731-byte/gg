...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> submit() async {
    ...
    if (isSignUp) {
      if (!validUsername) {
        return _setError('Username must be 3–30 characters: lowercase letters, numbers, _ or .');
      }
    ...
    try {
      ...
      if (isSignUp) {
        // The database trigger creates the profile atomically and enforces
        // case-insensitive username uniqueness. Do not upsert the profile
        // from the client because email-confirmation users may not yet have
        ...
        if (response.session != null) {
          setState(() {
            notice = 'Account created as @$username.';
          });
          return;
        }
        ...
        setState(() {
          ...
          notice = 'Account created as @$username. We sent an 8-digit verification code to $email.';
        });
      } else {
  ...
  String _friendlyAuthError(String message) {
    ...
    if (lower.contains('username') && (lower.contains('taken') || lower.contains('duplicate'))) {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          codeSent = true;
          forgotPassword = true;
          notice = 'A fresh 8-digit password-reset OTP was sent to $email.';
        });
      }
    } on AuthException catch (e) {
      if (mounted) setState(() => error = e.message);
    } catch (_) {
      if (mounted) setState(() => error = 'Could not send the password-reset OTP. Please try again.');
    } finally {
      if (mounted) setState(() => loading = false);
    }
  }
  ...
  Future<void> verifyEmail() async {
    ...
    if (forgotPassword && password.length < 8) {
      return _setError('Enter a new password of at least 8 characters.');
    }
    if (forgotPassword && password != confirmPassword) {
      return _setError('New passwords do not match.');
    }
    ...
    try {
      ...
      await auth.verifyOTP(
        email: email,
        token: otp,
        type: loginChallenge || forgotPassword ? OtpType.email : OtpType.signup,
      );
      ...
      if (forgotPassword) {
        ...
        setState(() {
          ...
          otpController.clear();
          notice = 'Password changed successfully. You are now signed in.';
        });
      }
    } on AuthException catch (e) {
      if (mounted) setState(() => error = e.message);
  ...
  Future<void> resendVerification() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => notice = forgotPassword
            ? 'A new 8-digit password-reset OTP was sent.'
            : loginChallenge
  ...
  void startForgotPassword() => setState(() {
        forgotPassword = true;
        isSignUp = false;
        codeSent = false;
        loginChallenge = false;
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    codeSent
                        ? (forgotPassword ? 'Reset your password' : loginChallenge ? 'Verify your login' : 'Verify your email address')
                        : forgotPassword
                  ...
                  if (codeSent) ...[
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: '8-digit ${forgotPassword ? 'password-reset OTP' : loginChallenge ? 'login OTP' : 'verification code'}',
                        hintText: '12345678',
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      ...
                      label: Text(loading
                          ? 'Please wait...'
                          : codeSent
                              ? (forgotPassword ? 'Verify & reset password' : 'Verify & continue')
                              : forgotPassword
                                  ? 'Send OTP'
                                  : isSignUp
                                      ? 'Create account'
                  ...
                  if (codeSent)
                    ...
                    TextButton(
                      onPressed: loading ? null : startForgotPassword,
                  ...
                  if (isSignUp && !codeSent)
                    Text(
                      'Username must be unique. Use 3–30 lowercase letters, numbers, _ or .',
                      textAlign: TextAlign.center,
