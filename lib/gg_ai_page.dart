...
class _GgAiPageState extends State<GgAiPage> {
  ...
  final conversationId = Supabase.instance.client.auth.currentUser == null
      ? 'local'
  ...
  Future<void> _speak(String text) async {
    final style = voiceStyles[selectedVoice];
    await tts.setLanguage('en-US');
    await tts.setPitch(style.pitch);
    await tts.setSpeechRate(style.rate);
  ...
  Future<void> _attach() async {
    ...
    if (bytes != null) {
      ...
      try {
        await supabase.storage.from('chat-media').uploadBinary(path, bytes, fileOptions: FileOptions(upsert: false));
        setState(() => messages.add({'role': 'user', 'content': '📎 ${file.name}', 'attachment': path}));
      } catch (e) {
  ...
  IconData _modeIcon(String value) {
    switch (value) { case 'Code': return Icons.code_rounded; case 'Debug': return Icons.bug_report_rounded; case 'Explain': return Icons.school_rounded; default: return Icons.auto_awesome_rounded; }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [CircleAvatar(radius: 20, child: Icon(_modeIcon(mode))), const SizedBox(width: 10), const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('GG AI', style: TextStyle(fontWeight: FontWeight.w900)), Text('GPT-OSS 120B • Groq', style: TextStyle(fontSize: 11))])]),
        actions: [
          ...
          child: messages.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    return Align(alignment: isUser ? Alignment.centerRight : Alignment.centerLeft, child: Container(constraints: const BoxConstraints(maxWidth: 380), margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.fromLTRB(14, 12, 10, 8), decoration: BoxDecoration(color: isUser ? scheme.primary : scheme.surfaceContainerHighest, borderRadius: BorderRadius.circular(20)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(text, style: TextStyle(color: isUser ? scheme.onPrimary : scheme.onSurface, height: 1.4)), if (message['attachment'] != null) Padding(padding: const EdgeInsets.only(top: 6), child: Text('📎 Attachment uploaded', style: TextStyle(color: isUser ? scheme.onPrimary : scheme.primary, fontWeight: FontWeight.w700))), if (!isUser) Row(children: [IconButton(tooltip: 'Listen', onPressed: () => _speak(text), icon: const Icon(Icons.volume_up_rounded, size: 19)), IconButton(tooltip: 'Copy', onPressed: () => _copy(text), icon: const Icon(Icons.copy_rounded, size: 18)), IconButton(tooltip: 'Share', onPressed: () => _share(text), icon: const Icon(Icons.share_rounded, size: 18)), IconButton(tooltip: 'Like', onPressed: () => _feedback(index, 1), icon: const Icon(Icons.thumb_up_alt_outlined, size: 18)), IconButton(tooltip: 'Dislike', onPressed: () => _feedback(index, -1), icon: const Icon(Icons.thumb_down_alt_outlined, size: 18))])])));
                  },
                ),
        ),
