...
Future<void> main() async {
  ...
  const url = 'https://vihbsfrwnslnmheowkhy.supabase.co';
  const key = 'sb_publishable_j8gV4-PeFte1RMgl759uQQ_KrM_3vzK';
...
class GGApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        theme: ThemeData(useMaterial3: true, colorSchemeSeed: Colors.indigo, brightness: Brightness.light,
          inputDecorationTheme: const InputDecorationTheme(filled: true, border: OutlineInputBorder(borderSide: BorderSide.none))),
        darkTheme: ThemeData(useMaterial3: true, colorSchemeSeed: Colors.indigo, brightness: Brightness.dark,
          inputDecorationTheme: const InputDecorationTheme(filled: true, border: OutlineInputBorder(borderSide: BorderSide.none))),
        themeMode: ThemeMode.system, home: const AuthGate());
}
...
class _LoginPageState extends State<LoginPage> {
  ...
  bool signUp = false, loading = false, obscure = true; String? error;
  Future<void> submit() async {
    if (!email.text.trim().contains('@') || password.text.length < 6) { setState(() => error = 'Enter a valid email and a password of at least 6 characters.'); return; }
    ...
    catch (_) { if (mounted) setState(() => error = 'Something went wrong. Please try again.'); }
    finally { if (mounted) setState(() => loading = false); }
  }
  Future<void> resetPassword() async {
    final value = email.text.trim(); if (!value.contains('@')) { setState(() => error = 'Enter your email first.'); return; }
    setState(() => loading = true);
    try { await Supabase.instance.client.auth.resetPasswordForEmail(value); if (mounted) setState(() => error = 'Password reset email sent.'); }
    on AuthException catch (e) { if (mounted) setState(() => error = e.message); }
    finally { if (mounted) setState(() => loading = false); }
  }
  @override void dispose() { email.dispose(); password.dispose(); super.dispose(); }
  @override
  Widget build(BuildContext context) => Scaffold(body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(
    constraints: const BoxConstraints(maxWidth: 430), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 18), Text('GG Messenger', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.w800)),
      const SizedBox(height: 6), Text(signUp ? 'Create your account' : 'Private, fast and realtime messaging'), const SizedBox(height: 30),
      ...
      if (!signUp) Align(alignment: Alignment.centerRight, child: TextButton(onPressed: loading ? null : resetPassword, child: const Text('Forgot password?'))), const SizedBox(height: 8),
...
class _HomePageState extends State<HomePage> {
  int tab = 0; late final PresenceService presence; Set<String> onlineUsers = <String>{};
  ...
  @override Widget build(BuildContext context) {
    final pages = [ChatsPage(onlineUsers: onlineUsers), const PlaceholderPage(title: 'Updates', icon: Icons.circle_outlined), const PlaceholderPage(title: 'Calls', icon: Icons.call_outlined)];
    return Scaffold(
...
class ChatsPage extends StatelessWidget {
  final Set<String> onlineUsers; const ChatsPage({super.key, this.onlineUsers = const <String>{}});
  @override Widget build(BuildContext context) => StreamBuilder<List<Map<String, dynamic>>>(stream: Supabase.instance.client.from('conversations').stream(primaryKey: ['id']).order('updated_at', ascending: false), builder: (context, snapshot) {
    if (snapshot.hasError) return Center(child: Text('Unable to load chats: ${snapshot.error}'));
    final chats = snapshot.data ?? []; if (chats.isEmpty) return const Center(child: Text('No chats yet\nTap New chat to start a conversation.', textAlign: TextAlign.center));
    return ListView.separated(padding: const EdgeInsets.only(top: 8, bottom: 100), itemCount: chats.length, separatorBuilder: (_, __) => const Divider(height: 1, indent: 76), itemBuilder: (context, i) {
      final chat = chats[i]; final title = (chat['title'] ?? 'Conversation').toString(); final initial = title.isEmpty ? '?' : title.substring(0, 1).toUpperCase(); final participantId = chat['other_user_id']?.toString(); final isOnline = participantId != null && onlineUsers.contains(participantId);
      return ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), leading: Stack(clipBehavior: Clip.none, children: [CircleAvatar(radius: 26, child: Text(initial, style: const TextStyle(fontWeight: FontWeight.bold))), if (isOnline) Positioned(right: -1, bottom: 0, child: Container(width: 14, height: 14, decoration: BoxDecoration(color: Colors.green, shape: BoxShape.circle, border: Border.all(color: Theme.of(context).scaffoldBackgroundColor, width: 2))))]), title: Text(title, style: const TextStyle(fontWeight: FontWeight.w700)), subtitle: Text((chat['last_message'] ?? 'Tap to open').toString(), maxLines: 1, overflow: TextOverflow.ellipsis), trailing: const Icon(Icons.chevron_right), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChatPage(conversationId: chat['id'].toString(), title: title))));
    });
  });
}
...
class SettingsPage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final user = Supabase.instance.client.auth.currentUser; return Scaffold(appBar: AppBar(title: const Text('Settings')), body: ListView(children: [UserAccountsDrawerHeader(decoration: BoxDecoration(color: Theme.of(context).colorScheme.primaryContainer), currentAccountPicture: CircleAvatar(child: Text((user?.email ?? 'G').substring(0, 1).toUpperCase())), accountName: Text(user?.userMetadata?['display_name']?.toString() ?? 'GG User'), accountEmail: Text(user?.email ?? '')), const ListTile(leading: Icon(Icons.lock_outline), title: Text('Privacy'), subtitle: Text('Your chats are protected by Supabase authentication and RLS.')), const ListTile(leading: Icon(Icons.data_usage_outlined), title: Text('Data saver'), subtitle: Text('Text-first messaging and controlled media downloads.')), const ListTile(leading: Icon(Icons.notifications_none), title: Text('Notifications'), subtitle: Text('Realtime message events are handled by Supabase.')), ListTile(leading: const Icon(Icons.logout), title: const Text('Log out'), onTap: () => Supabase.instance.client.auth.signOut())])); }
}
...
class _AppSearchDelegate extends SearchDelegate<String> {
  ...
  @override Widget buildResults(BuildContext context) => _searchBody();
  @override Widget buildSuggestions(BuildContext context) => _searchBody();
  Widget _searchBody() => Center(child: Text(query.trim().isEmpty ? 'Search chats and people' : 'Search results for “$query”'));
}

class PlaceholderPage extends StatelessWidget {
  final String title; final IconData icon; const PlaceholderPage({required this.title, required this.icon, super.key});
