        ...
        home: const Scaffold(
          body: Center(
            child: Padding(
              ...
              child: Text(
                'Supabase is not configured.\n\nRun with --dart-define=SUPABASE_URL=... and --dart-define=SUPABASE_PUBLISHABLE_KEY=...',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('GG Messenger', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold)),
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: pages[tab],
      floatingActionButton: tab == 0 ? FloatingActionButton(onPressed: () {}, child: const Icon(Icons.chat_rounded)) : null,
...
class ChatsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => StreamBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (chats.isEmpty) return const Center(child: Text('No chats yet\nStart a conversation to see it here.', textAlign: TextAlign.center));
          return ListView.separated(
            itemCount: chats.length,
            separatorBuilder: (_, __) => const Divider(height: 1),
            itemBuilder: (context, i) {
              final chat = chats[i];
              return ListTile(leading: CircleAvatar(child: Text((chat['title'] ?? 'G').toString().substring(0, 1).toUpperCase())), title: Text(chat['title'] ?? 'Conversation'), subtitle: Text(chat['last_message'] ?? 'Tap to open'), trailing: const Icon(Icons.chevron_right));
            },
          );
        },
      );
...
class PlaceholderPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 64), const SizedBox(height: 12), Text(title, style: Theme.of(context).textTheme.headlineSmall)]));
}
