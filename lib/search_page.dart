...
class _SearchPageState extends State<SearchPage> with SingleTickerProviderStateMixin {
        ...
        appBar: AppBar(
          title: TextField(
            ...
            decoration: const InputDecoration(
              hintText: 'Search people, chats and messages',
          ...
          actions: [
            ...
            IconButton(onPressed: loading ? null : search, icon: const Icon(Icons.search)),
          ],
          bottom: TabBar(
            controller: tabs,
            tabs: [
  ...
  Widget _people() {
    if (people.isEmpty) return _empty('No people found', Icons.person_search_rounded);
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, index) {
        ...
        return ListTile(
          ...
          subtitle: Text((chat['last_message'] ?? 'No messages yet').toString(), maxLines: 1, overflow: TextOverflow.ellipsis),
      ...
      itemBuilder: (_, index) {
        ...
        return Card(
          child: ListTile(
            ...
            title: Text((message['body'] ?? '').toString(), maxLines: 2, overflow: TextOverflow.ellipsis),
            subtitle: Text('Conversation ${message['conversation_id'].toString().substring(0, 8)}…'),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChatPage(conversationId: message['conversation_id'].toString(), title: 'Conversation'))),
          ),
        );
      },
    );
  ...
  Widget _avatar(String? url, String name) {
    ...
    return CircleAvatar(child: Text(name.isEmpty ? '?' : name[0].toUpperCase()));
  }
}
