...
class _StatusPageState extends State<StatusPage> {
  ...
  Future<void> _loadProfiles() async {
    try {
      ...
      for (final row in List<Map<String, dynamic>>.from(rows)) {
        next[row['id'].toString()] = Map<String, dynamic>.from(row);
      }
  ...
  Future<void> _postText() async {
    ...
    await _runPost(() => supabase.from('statuses').insert({
  ...
  Future<void> _postMediaFromGallery() async {
    ...
    await _uploadBytes(file.bytes!, extension, isVideo);
  }
  ...
  Future<void> _runPost(Future<void> Function() action) async {
    ...
    try {
      await action();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Status posted for 24 hours')));
    } catch (_) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not post status. Check your connection.')));
    } finally {
        ...
        return CustomScrollView(physics: const BouncingScrollPhysics(), slivers: [
          SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.fromLTRB(16, 12, 16, 4), child: Card(child: ListTile(
            ...
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              IconButton(onPressed: _choosePrivacy, tooltip: 'Status privacy', icon: Icon(_privacy == 'everyone' ? Icons.public_rounded : Icons.lock_rounded)),
              IconButton(onPressed: posting ? null : _postText, icon: const Icon(Icons.edit_rounded)),
          ...
          if (recent.isEmpty) const SliverFillRemaining(hasScrollBody: false, child: Center(child: Text('No recent updates yet.')))
            ...
            itemBuilder: (_, i) {
              final status = recent[i];
              final id = status['user_id']?.toString();
              ...
              return Card(margin: const EdgeInsets.only(bottom: 10), child: ListTile(
                ...
                onTap: () => _openStories(recent, start: i),
              ));
            },
          )),
        ]);
...
class _Avatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: CircleAvatar(radius: radius, backgroundColor: scheme.primaryContainer, backgroundImage: image, child: image == null ? Text(name.isEmpty ? '?' : name.characters.first.toUpperCase(), style: TextStyle(fontWeight: FontWeight.w900, color: scheme.onPrimaryContainer)) : null),
    );
  }
}
