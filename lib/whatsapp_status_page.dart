...
class _WhatsAppStatusPageState extends State<WhatsAppStatusPage> {
  ...
  Future<void> _loadProfiles() async {
    ...
    try {
      ...
      // Statuses remain usable with fallback initials when profile lookup fails.
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _loadStatuses, tooltip: 'Refresh', icon: const Icon(Icons.refresh_rounded)),
        ],
      ),
      body: _loading
              ...
              child: ListView(
                ...
                children: [
                  ...
                  if (others.isNotEmpty) ...[
                    ...
                    ...others.map(_statusTile),
                  ] else
                    _emptyUpdates(),
                  if (_error != null) ...[
                    const SizedBox(height: 12),
                    Text('Some updates could not be loaded.', style: TextStyle(color: Theme.of(context).colorScheme.error)),
                  ],
                ],
              ),
            ),
    ...
    return ListTile(
      ...
      leading: Stack(
        ...
        children: [
          ...
          Positioned(right: -2, bottom: -1, child: CircleAvatar(radius: 10, backgroundColor: Theme.of(context).colorScheme.primary, child: const Icon(Icons.add, size: 14))),
        ],
      ),
      title: Text('My status', style: const TextStyle(fontWeight: FontWeight.w900)),
  ...
  Widget _avatar(String name, String url, {double radius = 28, bool border = false}) {
    final child = url.isNotEmpty
        ...
        : CircleAvatar(radius: radius, child: Text(name.isEmpty ? '?' : name.substring(0, 1).toUpperCase(), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 20)));
...
class StatusViewerPage extends StatelessWidget {
      ...
      body: Center(
        child: Padding(
          ...
          child: type == 'image' && mediaUrl != null
              ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [Expanded(child: InteractiveViewer(child: Image.network(mediaUrl!, fit: BoxFit.contain))), if (caption.isNotEmpty) _caption(context, caption)])
              : type == 'video'
                  ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.play_circle_outline_rounded, color: Colors.white, size: 80), const SizedBox(height: 12), const Text('Video status', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)), if (caption.isNotEmpty) _caption(context, caption)])
  ...
  Widget _caption(BuildContext context, String text) => Padding(padding: const EdgeInsets.only(top: 12), child: Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 17)));
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(children: [
        Expanded(child: Image.memory(bytes as dynamic, fit: BoxFit.contain)),
