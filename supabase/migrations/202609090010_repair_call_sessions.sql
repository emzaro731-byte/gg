-- Repair migration for GG Messenger calls.
-- Fixes PGRST205 when call_sessions was never applied to the linked Supabase project.
...
create policy "call members can read sessions"
...
-- Make call history realtime-aware.
do $$
begin
  if not exists (
...
-- Ask PostgREST/Supabase to reload its schema cache after migration.
